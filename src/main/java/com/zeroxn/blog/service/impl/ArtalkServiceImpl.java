...
import com.zeroxn.blog.mapper.ArtalkMapper;
import com.zeroxn.blog.service.ArtalkService;
...
public class ArtalkServiceImpl implements ArtalkService {
    @Autowired
    private ArtalkMapper artalkMapper;
    ...
    public List<ArtalkComment> getArtalkCommentList(Integer pageNum, Integer pageSize) {
        ...
        artalkCommentList.forEach(artalkComment -> {
            //查询页面标题和发表评论的用户名
            artalkComment.setPageTitle(artalkMapper.getArtalkPageTitleByPageKey(artalkComment.getPageKey()));
            artalkComment.setUsername(artalkMapper.getArtalkUserNameById(artalkComment.getUserId()));
